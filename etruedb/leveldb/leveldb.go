...
func configureOptions(customizeFn func(*opt.Options)) *opt.Options {
	...
	options := &opt.Options{
		...
		//DisableSeeksCompaction: true, // todo evm
	}
...
func (db *Database) NewBatch() etruedb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator(prefix []byte, start []byte) etruedb.Iterator {
...
func (b *batch) Replay(w etruedb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  etruedb.KeyValueWriter
	failure error
}
