...
// FastIndexerConfig includes a set of configs for chain indexers.
...
func NewChtIndexer(db etruedb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	...
	return core.NewChainIndexer(db, etruedb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func (c *ChtIndexerBackend) fetchMissingNodes(ctx context.Context, section uint64, root common.Hash) error {
	...
	r := &ChtRequest{ChtRoot: root, ChtNum: section - 1, BlockNum: section*c.sectionSize - 1, Config: c.odr.FastIndexerConfig()}
	for {
		err := c.odr.FastRetrieve(ctx, r)
		switch err {
		case nil:
			r.Proof.Store(batch)
			return batch.Write()
