...
// NodeConfig represents the collection of configuration values to fine tune the Getrue
...
type NodeConfig struct {
	...
	// EthereumEnabled specifies whether the node should run the Truechain protocol.
	EthereumEnabled bool

	// EthereumNetworkID is the network identifier used by the Truechain protocol to
...
// Node represents a Getrue Truechain node instance.
type Node struct {
	node *node.Node
}

// NewNode creates and configures a new Getrue node.
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	// Register the Truechain protocol if requested
	if config.EthereumEnabled {
		...
		if config.EthereumNetStats != "" {
			if err := rawStack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
				...
				return etruestats.New(config.EthereumNetStats, nil, lesServ)
			}); err != nil {
...
// GetEthereumClient retrieves a client to access the Truechain subsystem.
