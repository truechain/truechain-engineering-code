...
var (
	...
	fsMinFullBlocks   = 10              // Number of blocks to retrieve fully even in fast sync
)
...
type Downloader struct {
	...
	SyncStatsState       etrue.StateSyncStats
	...
	// for stateFetcher
	stateSyncStart chan *etrue.StateSync
	TrackStateReq  chan *etrue.StateReq
...
func New(mode SyncMode, stateDb ethdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer etrue.PeerDropFn, fdown *fastdownloader.Downloader) *Downloader {
	...
	dl := &Downloader{
		...
		stateSyncStart: make(chan *etrue.StateSync),
		SyncStatsState: etrue.StateSyncStats{
			Processed: rawdb.ReadFastTrieProgress(stateDb),
		},
		TrackStateReq: make(chan *etrue.StateReq),
	}
			...
			if fbNumLast > currentNum {
				...
				if height > 0 {
					for {
						...
						errs := d.fastDown.Synchronise(p.GetID(), hash, td, fastdownloader.SyncMode(mode), fbNum, height)
						if errs != nil {
