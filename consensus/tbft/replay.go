...
func (cs *ConsensusState) readReplayMessage(msg *TimedWALMessage, newStepCh chan interface{}) error {
	...
	case types.EventDataRoundState:
		log.Info("Replay: New Step", "height", m.Height, "round", m.Round, "step", m.Step)
	...
	case msgInfo:
		...
		case *ProposalMessage:
			p := msg.Proposal
			log.Info("Replay: Proposal", "height", p.Height, "round", p.Round, "header",
				p.BlockPartsHeader, "pol", p.POLRound, "peer", peerID)
		case *BlockPartMessage:
			log.Info("Replay: BlockPart", "height", msg.Height, "round", msg.Round, "peer", peerID)
		case *VoteMessage:
			v := msg.Vote
			log.Info("Replay: Vote", "height", v.Height, "round", v.Round, "type", v.Type,
				"blockID", v.BlockID, "peer", peerID)
		}

		cs.handleMsg(m)
	case timeoutInfo:
		log.Info("Replay: Timeout", "height", m.Height, "round", m.Round, "step", m.Step, "dur", m.Duration)
...
func (cs *ConsensusState) catchupReplay(csHeight uint64) error {
	...
	if err == io.EOF {
		log.Error("Replay: wal.group.Search returned EOF", "#ENDHEIGHT", csHeight-1)
	} else if err != nil {
		return err
	}
	if !found {
	...
	log.Info("Catchup by replaying consensus messages", "height", csHeight)
	...
	for {
		...
		if err == io.EOF {
			...
			log.Debug("data has been corrupted in last height of consensus WAL", "err", err, "height", csHeight)
