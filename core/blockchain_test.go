...
func testFastBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
...
// the database if successful.
func testFastHeaderChainImport(chain []*types.Header, blockchain *BlockChain) error {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	var (
		...
		address = crypto.PubkeyToAddress(key.PublicKey)
		funds   = big.NewInt(1000000000)
		gspec   = &fastchain.Genesis{Config: params.TestChainConfig, Alloc: types.GenesisAlloc{address: {Balance: funds}}}
		genesis = gspec.MustCommit(gendb)
	)
	height := uint64(1024)
...
func TestLogReorgs(t *testing.T) {
	...
	var (
		...
		gspec   = &fastchain.Genesis{Config: params.TestChainConfig, Alloc: types.GenesisAlloc{addr1: {Balance: big.NewInt(10000000000000)}}}
		genesis = gspec.MustCommit(db)
		signer  = types.NewEIP155Signer(gspec.Config.ChainID)
	)
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	db := ethdb.NewMemDatabase()
	genesis := new(fastchain.Genesis).MustCommit(db)
	...
	new(fastchain.Genesis).MustCommit(diskdb)
