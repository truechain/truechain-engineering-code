...
func (b *FastBlockGen) AddTxWithChain(bc *FastBlockChain, tx *types.Transaction) {
	...
	receipt, _, err := FastApplyTransaction(b.config, bc, nil, b.gasPool, b.statedb, b.header, tx, &b.header.GasUsed, vm.Config{})
	if err != nil {
		panic(err)
	}
...
func GenerateFastChain(config *params.ChainConfig, parent *types.FastBlock, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *FastBlockGen)) ([]*types.FastBlock, []types.Receipts) {
	...
	blocks, receipts := make(types.FastBlocks, n), make([]types.Receipts, n)
	genblock := func(i int, parent *types.FastBlock, statedb *state.StateDB) (*types.FastBlock, types.Receipts) {
