...
func (tx *Transaction) Info() string {
	str := ""
	if tx != nil {
		str += fmt.Sprintf("nonce=%v,Price=%v,GasLimit=%v,Recipient=%v, Amount=%v,Payload=%v,Payer=%v,fee=%v,v=%v,r=%v,s=%v",
			tx.data.AccountNonce, tx.data.Price, tx.data.GasLimit, common.Bytes2Hex(tx.data.Recipient[:]), tx.data.Amount,
			tx.data.Payload, common.Bytes2Hex(tx.data.Payer[:]), tx.data.Fee,
