...
//func generateSnapshot(diskdb etruedb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash) *diskLayer {
...
func journalProgress(db etruedb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
//	var snapNodeCache etruedb.KeyValueStore
//	if len(result.keys) > 0 {
...
//		default:
//		}
//		if batch.ValueSize() > etruedb.IdealBatchSize || abort != nil {
...
//			// investigate if this can be avoided, this will be very costly since it
