...
func (bc *SnailBlockChain) GetFruitsHash(header *types.SnailHeader, fruits []*types.SnailBlock) common.Hash {
	if bc.chainConfig.IsTIP5(header.Number) {
		...
		log.Warn("GetFruitsHash", "number", header.Number, "DeriveSha hash", types.DeriveSha(types.FruitsHeaders(types.FruitsHeaders(headers))), "header.FruitsHash", header.FruitsHash, "len", len(fruits))
		return types.DeriveSha(types.FruitsHeaders(types.FruitsHeaders(headers)))
	}
	log.Warn("GetFruitsHash", "number", header.Number, "DeriveSha hash", types.DeriveSha(types.Fruits(fruits)), "header.FruitsHash", header.FruitsHash, "len", len(fruits))
