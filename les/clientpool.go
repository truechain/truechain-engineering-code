...
type clientPool struct {
	...
	closed     bool
	removePeer func(enode.ID, uint32)
...
func newClientPool(db etruedb.Database, freeClientCap uint64, queueLimit int, clock mclock.Clock, removePeer func(enode.ID, uint32)) *clientPool {
	pool := &clientPool{
		db:              db,
		clock:           clock,
...
func (f *clientPool) dropClient(e *clientInfo, now mclock.AbsTime, kick bool) {
	...
	if kick {
		...
		f.removePeer(e.id, public.DropClientCall)
	} else {
