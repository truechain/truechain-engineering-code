...
// HDScheme is the protocol scheme prefixing account and wallet URLs.
const HDScheme = "hdwallet"
...
func (w *Wallet) URL() accounts.URL {
	return accounts.URL{
		Scheme: HDScheme,
		Path:   fmt.Sprintf("%x", w.seed[1:5]), // Byte #0 isn't unique; 1:5 covers << 64K cards, bump to 1:9 for << 4M
	}
}
...
func (w *Wallet) SelfDerive(base accounts.DerivationPath, chain truechain.ChainStateReader) {
...
func (w *Wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	w.cacheMu.RLock() // Comms have own mutex, this is for the state fields
	defer w.cacheMu.RUnlock()
	// Make sure the requested account is contained within
	path, ok := w.paths[account.Address]
	if !ok {
	...
	msg, err := signedTx.AsMessage(types.NewTIP1Signer(chainID))
	if err != nil {
		return nil, err
	}
